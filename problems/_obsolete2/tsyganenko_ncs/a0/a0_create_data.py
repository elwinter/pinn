...
"""Create data for the a0 problem.
...
import pinn.common
import problems.tsyganenko_ncs.a0.a0 as p


# Program constants

# Program description
DESCRIPTION = 'Create data for the a0 problem.'
...
def main():
    ...
    header = (
        f"# {p.independent_variable_names[p.ifP]}"
        f" {p.independent_variable_names[p.ifBz]}"
        f" {p.dependent_variable_names[p.ia0]}"
    )
    print(header)
    ...
    for _fP in fP:
        for _fBz in fBz:
            _a0 = p.a0_empirical(_fP, _fBz)
            print(f"{_fP} {_fBz} {_a0}")
