"""Problem definition file for simple coupled ODE pair (Lagaris problem 4).

This is a pair of first-order, nonlinear ODE, defined on the domain [0, 3],
with the initial conditions Y1(0) = 0, Y2(0) = 1.
...
dependent_variable_names = ['Y1', 'Y2']
...
iY1 = dependent_variable_index['Y1']
iY2 = dependent_variable_index['Y2']

# Labels for dependent variables (may use LaTex) - use for plots.
dependent_variable_labels = ['$\Y_1$', '$Y_2$']
...
# @tf.function
def ode_Y1(X, Y, delY):
    """Differential equation for Y1.

    Evaluate the differential equation for Y1.
    ...
    (Y1, Y2) = Y
    (delY1, delY2) = delY
    dY1_dx = tf.reshape(delY1[:, ix], (nX, 1))
    dY2_dx = tf.reshape(delY2[:, ix], (nX, 1))
...
# @tf.function
def ode_Y2(X, Y, delY):
    """Differential equation for Y2.

    Evaluate the differential equation for Y2.
...
def Y1_analytical(x):
    """Analytical solution for Y1.

    Analytical solution to Y1.
...
def Y2_analytical(x):
    """Analytical solution for Y2.

    Analytical solution to Y2.
