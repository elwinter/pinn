...
"""Create plots for pinn0 results for the lagaris04 problem.
...
# Program description
DESCRIPTION = "Create plots for pinn0 results for lagaris04 problem."

# Name of problem
PROBLEM_NAME = "lagaris04"
...
def pinn0_plots(args: dict):
    """Make standard pinn0 plots for lagaris04.
    ...
    # Create the plot.
    fig = make_loss_plot(Le)

    # Save the plot to a PNG file.
    path = os.path.join(output_path, "Le.png")
    if verbose:
        print(f"Saving {path}.")
    plt.savefig(path)
    plt.close()
    ...
    for iv in range(p.n_var):
        ...
        path = os.path.join(results_path, "models", f"{last_epoch:06d}",
                            variable_name)
        models[iv] = tf.keras.models.load_model(path)

    # Compute predicted, analytical, and error values.
    X = XY_data[:, p.ix]
    Yp = [model(X).numpy().reshape(X.shape[0]) for model in models]
    Ya = [f(X) for f in p.Y_analytical]
    Ye = [p - a for (p, a) in zip(Yp, Ya)]
