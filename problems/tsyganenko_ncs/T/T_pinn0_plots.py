...
"""Create plots for pinn0 results for T problem.
...
# Program description
DESCRIPTION = 'Create plots for pinn0 results for T problem.'
...
def main():
    ...
    fig.suptitle(
        f"Loss function evolution for {p.dependent_variable_labels[p.iT]}"
    )
    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        line = f.readline()  # fP
        line = f.readline()  # fPmin fPmax nfP
        line = line[2:].rstrip()
        fields = line.split(' ')
        fPmin = float(fields[0])
        fPmax = float(fields[1])
        nfP = int(fields[2])
    ...
    # figure.
    variable_name = p.dependent_variable_names[p.iT]
    if verbose:
    ...
    # solutions, and error.
    fP_train = training_data[:, 0]
    T_trained = model(fP_train).numpy().reshape(fP_train.shape)
    T_empirical = p.T_empirical(fP_train)
    T_error = T_trained - T_empirical

    # Compute the RMS error.
    T_rmserr = np.sqrt(np.sum(T_error**2)/T_error.size)
    ...
    # Plot the data, then add the legend.
    ax.plot(fP_train, T_trained, label="$T$ (trained)")
    ax.plot(fP_train, T_empirical, label="$T$ (empirical)")
    ax.plot(fP_train, T_error, label="$T$ error")
    ax.legend()

    # Add a plot title with the RMS error.
    text = f"RMS error = {T_rmserr:.2E}"
    ax.set_title(text)

    # Save the plot to a PNG file.
    path = os.path.join(output_path, 'T.png')
    fig.savefig(path)
    plt.close(fig)
    if verbose:
        print(f"Saved figure as {path}.")
