...
def main():
    ...
    XY_data = np.loadtxt(data_path, dtype=precision, skiprows=1, delimiter=',')
    if debug:
    ...
    # # Create a model for each variable and parameter.
    models = []
    if verbose:
    ...
    for (i, v) in enumerate(p.dependent_variable_names):
        ...
        model = common.build_model(n_layers, H, activation, v)
        if debug:
            print(f"model = {model}", flush=True)
        models.append(model)
    for (i, param) in enumerate(p.parameter_names):
        if verbose:
            print(f"Creating model for {param}.", flush=True)
        model = common.build_model(n_layers, H, activation, param)
        if debug:
            print(f"model = {model}", flush=True)
        models.append(model)
    if debug:
    ...
    # # Create loss histories for each equation as Python lists.
    loss = {}
    for n in p.de_names:
        loss[n] = {}
        loss[n]["residual"] = []
    ...
    for epoch in range(max_epochs):
        ...
        with tf.GradientTape(persistent=True) as tape0:
            with tf.GradientTape(persistent=True) as tape1:
                ...
                # are the values of the dependent variables Y and parameters P
                # to use in the differential equations G.
                # Y_train_model is a list of tf.Tensor objects.
                # There are p.n_var+p.n_parameters Tensors in the list (one per model).
                ...
                # are the values of the dependent variables Y and parameters P
                # to use when comparing to the supplied data.
                # Y_data_model is a list of tf.Tensor objects.
                # There are p.n_var+p.n_parameters Tensors in the list (one per model).
                # Each Tensor has shape (n_data, 1).
                Y_data_model = [tf.Variable(model(X_data_tf)) for model in models]
                if debug:
                    print(f"Y_data_model = {Y_data_model}", flush=True)

                # <HACK>
                # Use the mean of the last model for alpha.
                # alpha_mean = tf.math.reduce_mean(Y_data_model[-1])
                # if debug:
                #     print(f"alpha_mean = {alpha_mean}")
                # Y_data_model[-1][...].assign(alpha_mean)
                # if debug:
                #     print(f"alpha = {Y_data_model[-1]}")
                # </HACK>

            # Compute the gradients of the dependent variables wrt independent
            # variables for the training points. These are the values of the partial
            ...
            for i in range(p.n_var):
                dYdX = tape1.gradient(Y_train_model[i], X_train_tf)
                dY_dX_train_model.append(dYdX)
            if debug:
                print(f"dY_dX_train_model = {dY_dX_train_model}", flush=True)

            # Extract the parameters.
            P_train_model = []
            for i in range(p.n_parameters):
                param = Y_train_model[p.n_var + i]
                P_train_model.append(param)
            if debug:
                print(f"P_train_model = {P_train_model}", flush=True)
            ...
            # There are p.n_var Tensors in the list (one per model).
            # Each Tensor has shape (n_train, 1).
            G_train_model = [f(X_train_tf, Y_train_model[:p.n_var], dY_dX_train_model,
                               P_train_model) for f in p.de]
            if debug:
            ...
            # training points for each equation.
            # L_res_per_equation is a list of Tensor objects.
            # There are p.nde Tensors in the list (one per equation).
            ...
            if debug:
                print(f"L_res_per_equation = {L_res_per_equation}", flush=True)

            # Compute the errors in the predicted values for the dependent variables
            # at the data points.
            # E_data_per_variable is a list of tf.Tensor objects.
            ...
            if debug:
                print(f"E_data_per_variable = {E_data_per_variable}", flush=True)

            # Compute the loss functions for the data points for each
            # model.
            # L_data_per_variable is a list of Tensor objects.
            # There are p.n_var Tensors in the list (one per model).
            # Each Tensor has shape () (scalar).
            L_data_per_variable = [
                tf.math.sqrt(tf.reduce_sum(E**2)/n_data) for E in E_data_per_variable
            ]
            if debug:
                print(f"L_data_per_variable = {L_data_per_variable}", flush=True)

            # # Compute the weighted aggregate loss function per model.
            # L_per_model = [
            #     w_res*L1 + w_data*L2 for (L1, L2)
            #     in zip(L_res_per_equation, L_data_per_variable)
            # ]
            # if debug:
            #     print(f"L_per_model = {L_per_model}", flush=True)

            # Compute the aggregated residual loss function.
            L_res = tf.math.reduce_sum(L_res_per_equation)
            if debug:
                print(f"L_res = {L_res}", flush=True)

            # Compute the aggregated data loss function.
            L_data = tf.math.reduce_sum(L_data_per_variable)
            if debug:
            ...
            if verbose:
                print(f"Epoch = {epoch}, L = {L}", flush=True)

    #         # Save the losses for this epoch.
    #         # for (i, v) in enumerate(p.dependent_variable_names):
    #         #     loss[v]["residual"].append(L_res_per_equation[i].numpy())
    #         #     loss[v]["data"].append(L_data_per_variable[i].numpy())
    ...
    for model in models:
        ...
        model.save(path)
        path = os.path.join(output_dir, "models", f"model_{model.name}.txt")
        old_stdout = sys.stdout
        with open(path, "w") as f:
            sys.stdout = f
            model.summary()
