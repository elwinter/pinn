...
# Default maximum and minimum values for divB.
DEFAULT_DIVBMIN = -0.001
DEFAULT_DIVBMAX = 0.001
...
def create_command_line_parser():
    ...
    parser.add_argument(
        "--divBmax", type=float, default=DEFAULT_DIVBMAX,
        help="Maximum divB value to plot (default: %(default)s)."
    )
    parser.add_argument(
        "--divBmin", type=float, default=DEFAULT_DIVBMIN,
...
def main():
    ...
    debug = args.debug
    divBmax = args.divBmax
    divBmin = args.divBmin
    epoch = args.epoch
    nt = args.nt
    nx = args.nx
    ny = args.ny
    ...
    if debug:
        print(f"args = {args}")
        print(f"debug = {debug}")
        print(f"divBmax = {divBmax}")
        print(f"divBmin = {divBmin}")
        print(f"epoch = {epoch}")
        print(f"nt = {nt}")
        print(f"nx = {nx}")
        print(f"ny = {ny}")
    ...
    for i in range(nt):
        ...
        txy_train = tf.Variable(txy[i0:i1])
        with tf.GradientTape(persistent=True) as tape1:
            Bx = model_Bx(txy_train)
            By = model_By(txy_train)
        dBx_dx = tape1.gradient(Bx, txy_train)[:, 1].numpy()
        dBy_dy = tape1.gradient(By, txy_train)[:, 2].numpy()
        divB = dBx_dx + dBy_dy
