...
"""Perform a set of pinn2.py runs.
...
options['problem_name'] = 'lagaris03'
options['problem_root'] = os.path.join(
    options['pinn_root'], 'problems',
    options['problem_class'], options['problem_name']
)
options['pinn2_problem_path'] = os.path.join(
    options['problem_root'], f"{options['problem_name']}.py"
)
options['pinn2_data_path'] = os.path.join(
    options['problem_root'], 'data', f"{options['problem_name']}_data.dat"
)
options['pinn2_training_path'] = os.path.join(
    options['problem_root'], 'data', f"{options['problem_name']}_021_training_grid.dat"
)
options['pinn2_results_dir'] = f"{options['problem_name']}-pinn2"
options['pinn2_plot_cmd'] = os.path.join(
    options['problem_root'], f"{options['problem_name']}_pinn2_plots.py"
)
...
options['pinn2_activation'] = 'sigmoid'
options['pinn2_debug'] = ''
options['pinn2_learning_rate'] = 0.01
options['pinn2_load_model'] = ''
options['pinn2_max_epochs'] = 10000
options['pinn2_n_hid'] = 10
options['pinn2_n_layers'] = 1
options['pinn2_nogpu'] = ''
options['pinn2_precision'] = 'float32'
options['pinn2_save_model'] = -1
options['pinn2_seed'] = None
options['pinn2_verbose'] = '--verbose'
options['pinn2_w_data'] = None

# Read and create the PBS script template.
PBS_TEMPLATE_FILE = os.path.join(
    options['pinn_root'], 'templates', 'pinn2_template.pbs'
)
...
def main():
    ...
    print(f"Problem definition file: {options['pinn2_problem_path']}")
    print(f"Problem data file: {options['pinn2_data_path']}")
    print(f"Problem training points file: {options['pinn2_training_path']}")
    print(f"Plotting script for results: {options['pinn2_plot_cmd']}")
    ...
    for w in WS:
        ...
        for run in range(N_RUNS):
            ...
            options['pinn2_seed'] = seed
            options['pinn2_w_data'] = w
